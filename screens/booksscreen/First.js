...
const FirstBookScreen = ({ onMenuPress, isMenuVisible, onCloseMenu, onNavigate, currentScreen, onBack, showBack }) => {
  ...
  const handleBookPress = (book) => {
    if (!hasAccess) {
      ...
      // Use the pseudo-route format expected by App.js and BookReaderScreen
    ...
    >
      <ScrollView style={styles.content} showsVerticalScrollIndicator={true}>
        <View style={styles.titleContainer}>
          <Text style={styles.title}>{t('series1')}</Text>
          <Text style={styles.subtitle}>{t('series1Subtitle')}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  bookCover: {
    width: '100%',
    height: 200, // Adjusted for proper book aspect ratio
    borderRadius: 8,
    marginBottom: 8,
    backgroundColor: '#eee',
  },
