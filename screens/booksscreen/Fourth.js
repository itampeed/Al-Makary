...
const FourthBookScreen = ({ onMenuPress, isMenuVisible, onCloseMenu, onNavigate, currentScreen, onBack, showBack }) => {
  ...
  const books = useMemo(() => allBooks.filter(b => b.series === '4'), [allBooks]);
  ...
  const loadData = async () => {
    try {
      // access check
      const access = await hasSeriesAccess('4');
      setHasAccess(access);
    } catch (error) {
    ...
    >
      <ScrollView style={styles.content} showsVerticalScrollIndicator={true}>
        <View style={styles.titleContainer}>
          <Text style={styles.title}>{t('series4')}</Text>
          <Text style={styles.subtitle}>{t('series4Subtitle')}</Text>
        </View>
